...
public class TrafficEdge
{
    ...
    public List<Vector2Int> touchedCells = new List<Vector2Int>();
    ...
    public bool HasEntrySpace(float vehicleLengthUnits, float gapUnits)
    {
        return HasSpaceAtDistance(0f, vehicleLengthUnits, gapUnits, null);
    }

    public bool HasSpaceAtDistance(float distance, float vehicleLengthUnits, float gapUnits, VehicleAI ignoredVehicle = null)
    {
        float clampedDistance = Mathf.Clamp(distance, 0f, totalLength);
        ...
        if (occupants != null)
        {
            foreach (VehicleAI occupant in occupants)
            {
                if (occupant == null || occupant == ignoredVehicle) continue;

                float occupantHalfLength = occupant.GetVehicleLengthUnits() * 0.5f;
                float safeGap = Mathf.Max(
                    Mathf.Max(0f, gapUnits),
                    occupant.GetMinimumFollowingGapUnits());
                float occupantMin = occupant.conveyorDistanceOnEdge - occupantHalfLength - safeGap;
                float occupantMax = occupant.conveyorDistanceOnEdge + occupantHalfLength + safeGap;
                if (IntervalsOverlap(candidateMin, candidateMax, occupantMin, occupantMax)) return false;
            }
        }

        if (reservations != null)
        {
            foreach (TrafficEdgeReservation reservation in reservations)
            {
                if (reservation == null ||
    ...
    public bool CanReserveInterval(
        float minDistance,
        float maxDistance,
        VehicleAI vehicle,
        float reservationGapUnits)
    {
        ...
        if (occupants != null)
        {
            foreach (VehicleAI occupant in occupants)
            {
                if (occupant == null || occupant == vehicle) continue;

                float halfLength = occupant.GetVehicleLengthUnits() * 0.5f;
                float additionalGap = Mathf.Max(
                    0f,
                    occupant.GetMinimumFollowingGapUnits() - Mathf.Max(0f, reservationGapUnits));
                float occupantMin = occupant.conveyorDistanceOnEdge - halfLength - additionalGap;
                float occupantMax = occupant.conveyorDistanceOnEdge + halfLength + additionalGap;
                if (IntervalsOverlap(intervalMin, intervalMax, occupantMin, occupantMax)) return false;
            }
        }
        ...
        int sequence)
    {
        ...
        if (reservations == null) reservations = new List<TrafficEdgeReservation>();

        TrafficEdgeReservation existing = reservations.Find(r => r != null && r.vehicle == vehicle);
        if (existing == null)
        {
        ...
        existing.minDistance = Mathf.Min(minDistance, maxDistance);
        existing.maxDistance = Mathf.Max(minDistance, maxDistance);
        existing.sequence = sequence;
    }

    public void RemoveReservation(VehicleAI vehicle)
    {
        if (reservations == null || vehicle == null) return;
    ...
    public VehicleAI GetVehicleAhead(VehicleAI vehicle)
    {
        if (vehicle == null || occupants == null) return null;

        for (int i = occupants.Count - 1; i >= 0; i--)
        {
            VehicleAI candidate = occupants[i];
            if (candidate == null || candidate == vehicle) continue;
            if (candidate.conveyorDistanceOnEdge > vehicle.conveyorDistanceOnEdge) return candidate;
        }

        return null;
    }

    public VehicleAI GetVehicleBehind(VehicleAI vehicle)
    {
        if (vehicle == null || occupants == null) return null;
